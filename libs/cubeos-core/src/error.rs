...
pub enum Error {
    ...
    WrongNoArgs,
    /// Wrong CommandID
    #[error("No Command with matching CommandID")]
    NoCmd,
    ...
    #[error("Poisoned RwLockReadGuard")]
    PoisonedRwLock,
    /// UART
    #[error("UART Error")]
    Uart(u8),
...
impl From<uart_rs::UartError> for Error {
    fn from(e: uart_rs::UartError) -> Error {
        match e {
            ...
            uart_rs::UartError::SerialError(s) => match s {
...
impl From<DieselError> for Error {
    fn from(e: DieselError) -> Error {
        match e {
            DieselError::InvalidCString(_) => Error::Diesel(0),
            DieselError::DatabaseError(_, _) => Error::Diesel(1),
            DieselError::NotFound => Error::Diesel(2),
            DieselError::QueryBuilderError(_) => Error::Diesel(3),
            DieselError::DeserializationError(_) => Error::Diesel(4),
            DieselError::SerializationError(_) => Error::Diesel(5),
            DieselError::RollbackTransaction => Error::Diesel(6),
            DieselError::AlreadyInTransaction => Error::Diesel(7),
            _ => Error::Diesel(8),
        }
    }
}
...
impl From<Error> for DieselError {
    fn from(e: Error) -> DieselError {
        match e {
            Error::Diesel(0) => DieselError::InvalidCString(std::ffi::CString::new(b"f\0oo".to_vec()).unwrap_err()),
            Error::Diesel(1) => DieselError::DatabaseError(diesel::result::DatabaseErrorKind::UniqueViolation, Box::<String>::new("".into())),
            Error::Diesel(2) => DieselError::NotFound,
            Error::Diesel(3) => DieselError::QueryBuilderError("".into()),
            Error::Diesel(4) => DieselError::DeserializationError("".into()),
            Error::Diesel(5) => DieselError::SerializationError(Box::new(DieselError::NotFound)),
            Error::Diesel(6) => DieselError::RollbackTransaction,
            Error::Diesel(7) => DieselError::AlreadyInTransaction,
            _ => DieselError::InvalidCString(std::ffi::CString::new(b"f\0oo".to_vec()).unwrap_err()),
        }
    }
}
